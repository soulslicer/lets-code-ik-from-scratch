...
# Setup a toy problem
target_input = {"j1": np.pi/2 + 1.0, "j2": np.pi/2 + 0.5}
print(target_input)
...
inputs = {"j1": 0.3, "j2": 0.3} # 0,0 fails with G
targets = {"ef": {"T": t, "w": w}, "j2": {"range": [-3,3], "w": 5}}
robot.fk(inputs, {}, viz = True)
time.sleep(1)
...
for u in range(0, x.shape[0]):
    for v in range(0, y.shape[0]):
        j1 = X[u,v]
        j2 = Y[u,v]
        total_losses, _, _ = robot.fk({"j1": j1, "j2": j2}, targets)
...
viz_targets = [[target_input["j1"], target_input["j2"]], [target_input["j1"]-np.pi*2, target_input["j2"]], [target_input["j1"], target_input["j2"]-np.pi*2]]
for vt in viz_targets:
    ax.plot(vt[0], vt[1], marker="o", markersize=3, markeredgecolor="green", markerfacecolor="green")
plt.ion()
def cb(input):
    ax.plot(input["j1"], input["j2"], marker="o", markersize=1, markeredgecolor="red", markerfacecolor="green")
    plt.pause(0.001)
